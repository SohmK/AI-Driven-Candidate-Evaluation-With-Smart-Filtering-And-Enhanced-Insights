...
genai.configure(api_key="AIzaSyC3mkOEhRFYp7L4-pY6Mvt0s35qeDSIJQ8")
...
app.config['MYSQL_USER'] = 'root'  # Adding our MySQL username in the .env file
app.config['MYSQL_PASSWORD'] = '12345'  # Adding our MySQL password in the .env file
app.config['MYSQL_DB'] = 'smarthire'  # Adding our database name in the .env file
app.config['SESSION_TYPE'] = 'filesystem' 
mysql = MySQL(app)

app.config['JWT_SECRET_KEY'] = 'BtechMajorProject2025rhfnjbgfbjldalkdcbhhcijodf3282'
jwt = JWTManager(app)
app.secret_key = '3w5rtfduionwecur90u0t94ntc58yrtijgi559m0yf9g'
...
def upload_resume():
    ...
    # print("Resume  extracted text is : ",resume_text)
    if not resume_text:
    ...
    print("gemini response on resume evalluation and skills percentage is ",gem_response)

    if gem_response:
        try:
            # Use regex to extract JSON block (inside `{ ... }`)
            json_match = re.search(r"\{.*\}", gem_response, re.DOTALL)
            if json_match:
                cleaned_json_str = json_match.group(0)
                gem_response_data = json.loads(cleaned_json_str)
            else:
...
def get_gemini_response(prompt):
    try:
        ...
        model = genai.GenerativeModel('gemini-2.0-flash')  # Ensure this matches your library's structure
        response = model.generate_content(prompt)  # This is an example; check actual method
        # print (response)
        return response.candidates[0].content.parts[0].text  # Adjust according to the actual response structure
    except Exception as e:
